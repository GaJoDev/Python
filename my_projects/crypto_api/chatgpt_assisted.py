...
def get_data_set():
    while True:
        ...
        user_option = input("[1] API request | [2] use existing data: ")
...
def process_data_list(data_list):
    for x in range(0, 2):
    # identify a record
        # These used ot be print statements but for easier debugging I've turned them into variables
        rank = data_list[x]['rank']
        symbol = data_list[x]['symbol']
        price = data_list[x]['quotes']['USD']['price']
        last_updated = data_list[x]['last_updated']
        print(rank)
        print(symbol)
        print(price)
        print(last_updated)
